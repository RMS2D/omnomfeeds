...
func (h *Handler) handleCheckout(w http.ResponseWriter, r *http.Request) {
	...
	h.analytics.Emit(u.ID, analytics.SessionFromRequest(r), analytics.EvProCheckoutStart, sess.ID, nil)
...
func (h *Handler) handleWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.dispatchEvent(r, &ev); err != nil {
		...
		// Returning 500 makes Stripe retry. Many errors are worth retrying;
		// truly malformed ones we return 200 to so retries don't loop.
...
func (h *Handler) onCheckoutCompleted(r *http.Request, obj json.RawMessage) error {
	...
	h.analytics.Emit(s.ClientReferenceID, "", analytics.EvProSubscribeSuccess, s.SubscriptionID, nil)
	return nil
}
...
func (h *Handler) onInvoicePaid(r *http.Request, obj json.RawMessage) error {
	...
	h.analytics.Emit(user.ID, "", analytics.EvProSubscribeRenew, inv.SubscriptionID, nil)
	return nil
}
